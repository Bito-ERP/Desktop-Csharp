...
internal class ReceiptResponse
{
    ...
    public Receipt Get(string organizationId)
    {
        return new Receipt
        {
            ...
            CustomerBeforeBalance = CustomerBeforeBalance.Select(it => it.Get()),
            CustomerAfterBalance = CustomerAfterBalance.Select(it => it.Get()),
            CustomerBeforeCashback = CashbackBeforeBalance.Select(it => it.Get()),
            CustomerAfterCashback = CashbackAfterBalance.Select(it => it.Get()),
